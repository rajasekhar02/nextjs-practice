...
async function fetchData({page}: { page: number }): Promise<ListCharacterResponse> {
  const URL = `http://localhost:3000/api?${new URLSearchParams(`page=${page}`)}`
  console.log(URL)
      ...
      <ul>
        {
          results.map(character => (
            <li>
              <h1 className="text-2xl font-medium text-gray-200">{character.name}</h1>
