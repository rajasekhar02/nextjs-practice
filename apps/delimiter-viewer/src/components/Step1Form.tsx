...
import { FormStages, Values } from "../app/types";
import TextArea from "./TextArea";
import { formStagesAtom, step1FormAtom } from "../app/state";
import { useAtomValue, ExtractAtomArgs, ExtractAtomResult } from "jotai";
...
const RadioButtonWithCustomInput = function () {
  ...
  const [customDelimiterField, metaCDF, helpersCDF] =
    useField("customDelimiter");
  let handleCustomDelimiterChange = function (
    newText: ChangeEvent<HTMLInputElement>,
  ) {
  ...
  // },[delimiterField])
  return (
    <RadioButton label="Custom" value={"custom"} key={`delimiter-radio-custom`}>
      <input
